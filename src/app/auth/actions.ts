...
function destination(formData: FormData) {
  ...
  return requested.startsWith("/") && !requested.startsWith("//") ? requested : "/biloo";
}
...
function normalizeEthiopianPhone(input: string) {
  ...
  const national = digits.startsWith("251") ? digits.slice(3) : digits.startsWith("0") ? digits.slice(1) : digits;
...
export async function signInAction(formData: FormData) {
  ...
  if (!email || !password) authError("/auth/login", "Email and password are required.");
...
export async function signInWithGoogleAction(formData: FormData) {
  ...
  if (error || !redirectUrl) authError("/", error?.message ?? "Google sign-in is not available right now.");
...
export async function signUpAction(formData: FormData) {
  ...
  if (firstName.length < 2 || fatherName.length < 2 || grandfatherName.length < 2) {
    authError("/auth/sign-up", "Enter your first name, father’s name and grandfather’s name.");
  }
  if (!/^[a-z][a-z0-9._]{2,29}$/.test(username)) {
    authError("/auth/sign-up", "Username must be 3–30 characters, start with a letter, and use only letters, numbers, dots or underscores.");
  }
  if (!phone) authError("/auth/sign-up", "Enter a valid Ethiopian mobile number.");
  if (!email || !email.includes("@")) authError("/auth/sign-up", "Enter a valid email address.");
  if (!region || city.length < 2 || address.length < 3) {
    authError("/auth/sign-up", "Select your region and enter your city and delivery address.");
  }
  if (password.length < 8) authError("/auth/sign-up", "Password must contain at least 8 characters.");
  if (password !== confirmPassword) authError("/auth/sign-up", "Passwords do not match.");
  if (!acceptedTerms) authError("/auth/sign-up", "Accept the BILOO terms and privacy notice to create your account.");
...
export async function updatePasswordAction(formData: FormData) {
  const password = value(formData, "password");
  if (password.length < 8) authError("/auth/update-password", "Password must contain at least 8 characters.");
  const supabase = await createClient();
